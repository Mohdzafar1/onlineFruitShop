...
const CartSidebar = ({ handleCloseCart, onUpdateQuantity }) => {
 ...
 const handleDelete=(proId)=>{
  ...
  return (
    <>
    {/* Sidebar */}
<div className="fixed inset-y-0 right-0 w-full max-w-md bg-white shadow-xl z-50 transform transition-transform duration-300 ease-in-out flex flex-col">
  {/* Header */}
      ...
      <div key={item._id} className="flex gap-4 py-4 border-b">
        ...
        <div className="flex-1">
           <div className="flex flex-rows justify-between">
             <div><h3 className="font-medium">{item.name}</h3></div>
              <div onClick={()=>handleDelete(item._id)}><MdDelete color="red"/></div>
           </div>
